...
#     
# 3. 刪除：
#     這一部分比我之前在寫LeetCode707的LinkedList還要更複雜了許多，因為多了left跟right要考慮，所以我還在研究要怎麼寫！這部分跟後續修改的部分會再補交！
#     
#     (已更新：）
#                        3
#                 3      8
...
#       -5   
#           3
#        這顆奇耙的樹可以幫我用來檢查delete的部分是怎麼進行的。
#        
#        總之我這個部分花了一天去思考，四個小時半才寫好code
#        
#        一邊寫到都想哭，因為太多東西需要考慮，雖然可以利用前面寫的搜尋跟新增的部分幫助我思考，不過這部分真的是太複雜了...
#        但我code上面註解都寫蠻詳細我怎麼想的
# 
# 4. 修改：
#     這部分我寫了兩個多小時...
#     modify 的話 顧名思義就是希望你把 val值為target 的點 數值改成 new_val 並且調整順序以符合 BST，但是 直接修改完的話 反而可能會因此打亂結構，因此這裡的作法我們換成先把所有符合條件的點都拿出來 之後再加回去，與前面 delete的做法差別在於，這一次我們需要計算一下總共刪掉幾個需要加回幾個，因為在delete裡面我一不小心全部做完了，所以得從另一個方面來計算數量。 現在手邊只有search 這樣可以找樹裡面是否有特定target 的函式。
#     但要計算數量的話怎麼辦呢？可以 一層一層的 來找，首先從 root 開始找起，如果有 那我們看 root 是不是 val符合條件，剩下的左半邊 跟右半邊 是不是符合條件，遞迴下去找並且統計數量。
#     
#     我在這部分為了要計算出他整棵樹總共有幾個需要改的數字，所以呢我在後面有多寫一個 count_val 的部分！！～
#     
#     然後將modify分成三個部分，再串起來：
